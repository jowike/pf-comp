...
src_path = "../data/03_intermediate/data_source/"
ds_path = "../data/4_features/"

model_dirpath = "../data/7_pixel_flipping/"
...
ds_listdir = [
    i for i in ds_listdir if i != "../data/4_features/MRMR_nfeatures_auto.csv"
]
...
def main():
    ...
    for ds_fname in ds_listdir:
        ...
        try:
            ...
            ds_long = ds_long.loc[
                : (
                    ds_long.loc[ds_long["reference_date"] == "reference_date"].index[0]
                    - 1
                )
            ]
            ds_long = convert_to_datetime(ds_long, ["reference_date"])
        ...
        match_substr = re.search(
            f"real_time_vintage_(.*)_{fs_method}_", ds_fname
        ).group(1)
        ...
        df = ds_long.drop(columns=["IsTarget"]).rename(
            columns={"value": "variable_value"}
        )
        ...
        for x in features:
            ...
            starting_points = np.random.choice(
                range(window_size + 1, series_length - window_size),
                num_starting_points,
                replace=False,
            )
            ...
            for start_index in starting_points:
                ...
                for model in estimators:
                    ...
                    model_output_fname = (
                        ...
                        .replace(
                            "df_",
                            f'fcst_{type(model).__name__}_{reference_date.strftime("%Y%m%d")}_',
                            1,
                        )
                        .replace(".csv", "")
                    ...
                    for ref_date in fcst_ref_dates:
                        ...
                        df_train.index = pd.DatetimeIndex(
                            df_train.index.values, freq="MS"
                        )
                        df_test.index = pd.DatetimeIndex(
                            df_test.index.values, freq="MS"
                        )
                        ...
                        try:
                            ...
                            coef_series = pd.Series(
                                np.round(
                                    model.base_estimator_.fit(X_train, y_train).coef_[
                                        0
                                    ],
                                    5,
                                ),
                    ...
                    if list(filter(re.compile(f".*perc_diff_lag").match, [y_code])):
                        ...
                        retransf_bench_bcst = bench_fr.make_inv_diff(
                            order=1, lagNum=offset
                        )
                    elif y_code == y_id:
                    ...
                    retransf_bench_bcst = retransf_bench_bcst.loc[
                        benchmark_backcast.index
                    ]
                    ...
                    with open(
                        os.path.join(model_dirpath, model_output_fname), "w"
                    ) as f:
